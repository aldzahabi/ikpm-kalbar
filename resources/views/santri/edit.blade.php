    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-green-100 overflow-hidden">
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 bg-brand-bg">
                ...
                <p class="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">Perbarui informasi data santri di bawah ini</p>
            </div>
            
            <!-- Form -->
            <form action="{{ route('santri.update', $santri->stambuk) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-4 sm:mb-6">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="stambuk" 
                            name="stambuk" 
                            value="{{ old('stambuk', $santri->stambuk) }}"
                            maxlength="5"
                            readonly
                            class="w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-300 rounded-lg bg-gray-50 text-gray-600 cursor-not-allowed text-sm"
                        >
                        <p class="mt-1 text-xs text-gray-500">Stambuk tidak dapat diubah</p>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="Kalimantan Barat" {{ old('provinsi', $santri->provinsi) == 'Kalimantan Barat' ? 'selected' : '' }}>Kalimantan Barat</option>
                            <option value="Kalimantan Tengah" {{ old('provinsi', $santri->provinsi) == 'Kalimantan Tengah' ? 'selected' : '' }}>Kalimantan Tengah</option>
                            <option value="Kalimantan Selatan" {{ old('provinsi', $santri->provinsi) == 'Kalimantan Selatan' ? 'selected' : '' }}>Kalimantan Selatan</option>
                            <option value="Kalimantan Timur" {{ old('provinsi', $santri->provinsi) == 'Kalimantan Timur' ? 'selected' : '' }}>Kalimantan Timur</option>
                            <option value="Kalimantan Utara" {{ old('provinsi', $santri->provinsi) == 'Kalimantan Utara' ? 'selected' : '' }}>Kalimantan Utara</option>
                            <option value="Jawa Barat" {{ old('provinsi', $santri->provinsi) == 'Jawa Barat' ? 'selected' : '' }}>Jawa Barat</option>
                            <option value="Jawa Tengah" {{ old('provinsi', $santri->provinsi) == 'Jawa Tengah' ? 'selected' : '' }}>Jawa Tengah</option>
                            <option value="Jawa Timur" {{ old('provinsi', $santri->provinsi) == 'Jawa Timur' ? 'selected' : '' }}>Jawa Timur</option>
                            <option value="Sumatera Barat" {{ old('provinsi', $santri->provinsi) == 'Sumatera Barat' ? 'selected' : '' }}>Sumatera Barat</option>
                            <option value="Sumatera Utara" {{ old('provinsi', $santri->provinsi) == 'Sumatera Utara' ? 'selected' : '' }}>Sumatera Utara</option>
                            <option value="Riau" {{ old('provinsi', $santri->provinsi) == 'Riau' ? 'selected' : '' }}>Riau</option>
                            <option value="Lainnya" {{ old('provinsi', $santri->provinsi) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('provinsi')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="daerah" 
                            name="daerah" 
                            value="{{ old('daerah', $santri->daerah) }}"
                            required
                            class="w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-200 focus:border-brand-primary transition-colors text-gray-800 placeholder-gray-400 text-sm"
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($pondokCabangList as $key => $name)
                                <option value="{{ $key }}" {{ old('pondok_cabang', $santri->pondok_cabang) == $key ? 'selected' : '' }}>{{ $key }} - {{ $name }}</option>
                            @endforeach
                        </select>
                        @error('pondok_cabang')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div class="mb-4 sm:mb-6">
                    ...
                    >{{ old('alamat', $santri->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4 sm:mb-6">
                    ...
                    >
                        ...
                        <option value="baru" {{ old('kenaikan_kelas', $santri->kenaikan_kelas) == 'baru' ? 'selected' : '' }}>Baru</option>
                        <option value="naik" {{ old('kenaikan_kelas', $santri->kenaikan_kelas) == 'naik' ? 'selected' : '' }}>Naik</option>
                        <option value="tidak_naik" {{ old('kenaikan_kelas', $santri->kenaikan_kelas) == 'tidak_naik' ? 'selected' : '' }}>Tidak Naik</option>
                        <option value="lulus" {{ old('kenaikan_kelas', $santri->kenaikan_kelas) == 'lulus' ? 'selected' : '' }}>Lulus</option>
                    </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div>
                            ...
                            <div class="mb-2 flex items-center space-x-3">
                                <img src="{{ asset('storage/' . $santri->foto_diri) }}" alt="Foto Diri" class="w-20 h-20 object-cover rounded-lg border">
                            ...
                            <p class="mt-1 text-xs text-gray-500">{{ $santri->foto_diri ? 'Upload untuk mengganti foto' : 'Format: JPG, PNG, WEBP. Maks 2MB' }}</p>
                            @error('foto_diri')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            @if($santri->foto_kk)
                            <div class="mb-2 flex items-center space-x-3">
                                <img src="{{ asset('storage/' . $santri->foto_kk) }}" alt="Foto KK" class="w-28 h-16 object-cover rounded-lg border">
                                <span class="text-xs text-green-600 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                    </svg>
                            ...
                            <p class="mt-1 text-xs text-gray-500">{{ $santri->foto_kk ? 'Upload untuk mengganti foto' : 'Format: JPG, PNG, WEBP. Maks 2MB' }}</p>
                            @error('foto_kk')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
