...
Formulir penambahan konten {{ ($type ?? 'slider') == 'slider' ? 'slider hero' : 'berita/informasi' }}
@endsection
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-green-100 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-brand-bg">
                ...
                <p class="text-sm text-gray-600 mt-1">Lengkapi formulir di bawah ini untuk menambahkan konten baru</p>
            </div>
            
            <!-- Form -->
            <form action="{{ route('landing-content.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="slider" {{ old('type', $type ?? '') == 'slider' ? 'selected' : '' }}>Slider Hero</option>
                            <option value="news" {{ old('type', $type ?? '') == 'news' ? 'selected' : '' }}>Berita/Informasi</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        @error('image')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">Format: JPG/PNG/WEBP, Maksimal 5MB. Ukuran rekomendasi: 1920x1080px</p>
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100">
                    <a 
                        href="{{ route('landing-content.index', ['tab' => $type ?? 'slider']) }}" 
