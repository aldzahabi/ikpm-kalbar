...
@section('title', 'Edit Rombongan - IKPM Gontor Pontianak')
@section('page-title', 'Edit Rombongan')
@section('page-subtitle', 'Perbarui data rombongan')

@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-green-100 p-6 md:p-8">
        <form action="{{ route('rombongan.update', $rombongan) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select 
                        ...
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition-colors text-gray-800"
                    >
                        <option value="Bus" {{ old('moda_transportasi', $rombongan->moda_transportasi) === 'Bus' ? 'selected' : '' }}>Bus</option>
                        <option value="Pesawat" {{ old('moda_transportasi', $rombongan->moda_transportasi) === 'Pesawat' ? 'selected' : '' }}>Pesawat</option>
                        <option value="Kapal" {{ old('moda_transportasi', $rombongan->moda_transportasi) === 'Kapal' ? 'selected' : '' }}>Kapal</option>
                    </select>
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="kapasitas" 
                        name="kapasitas" 
                        value="{{ old('kapasitas', $rombongan->kapasitas) }}"
                        required
                        min="1"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition-colors text-gray-800"
                        placeholder="40"
                    ...
                    @if($rombongan->santris()->count() > 0)
                        <p class="mt-1 text-xs text-gray-500">Jumlah santri terdaftar: {{ $rombongan->santris()->count() }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        value="{{ old('waktu_berangkat', $rombongan->waktu_berangkat->format('Y-m-d\TH:i')) }}"
                        required
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition-colors text-gray-800"
                    >
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="titik_kumpul" 
                        name="titik_kumpul" 
                        value="{{ old('titik_kumpul', $rombongan->titik_kumpul) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition-colors text-gray-800"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        value="{{ old('biaya_per_orang', $rombongan->biaya_per_orang) }}"
                        required
                        min="0"
                        step="0.01"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition-colors text-gray-800"
                ...
                <div>
                    ...
                    <select 
                        ...
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition-colors text-gray-800"
                    >
                        <option value="open" {{ old('status', $rombongan->status) === 'open' ? 'selected' : '' }}>Open</option>
                        <option value="closed" {{ old('status', $rombongan->status) === 'closed' ? 'selected' : '' }}>Closed</option>
                        <option value="departed" {{ old('status', $rombongan->status) === 'departed' ? 'selected' : '' }}>Departed</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
